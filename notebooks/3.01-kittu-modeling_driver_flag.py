...
def _():
    mo.md("""
    # Driver Delay Flag Prediction Model

    This notebook builds a **binary classification** model to predict whether a driver will be delayed.

    **Target:** `driver_delay_flag` (0 = No Delay, 1 = Delayed)

    **Class Imbalance:** 87% No Delay vs 13% Delayed (uses `class_weight="balanced"`)
...
@app.cell
def _():
    df_raw = pd.read_csv(PROCESSED_DATA_DIR / "features_driver_delay_flag.csv")
...
def _(final_pipe, tuned_pipe):
    ...
    save_dir = MODELS_DIR / "driver_delay_flag"
...
def _():
    ...
    mo.md("Configure the inputs below and click **Predict Delay** to get a prediction.")
    return
...
def _():
    ...
    predict_btn = mo.ui.run_button(label="Predict Delay")
...
def _(feature_cols, input_widgets, predict_btn, tuned_pipe):
    ...
    if predict_btn.value and tuned_pipe is not None:
        ...
        status_map = {0: "No Delay", 1: "Delayed"}
        pred_label = status_map.get(pred, str(pred))
        pred_md = (
            f"### Prediction: **{pred_label}** (class {pred})\n\n"
            f"**Delay Probability:** {proba[1]:.1%}\n\n"
            f"| Class | Probability |\n|-------|-------------|\n"
            f"| No Delay (0) | {proba[0]:.1%} |\n"
            f"| Delayed (1) | {proba[1]:.1%} |\n\n"
            f"**Inputs:**\n"
        ...
        pred_md = "Configure inputs and click **Predict Delay**."
    return (pred_md,)
