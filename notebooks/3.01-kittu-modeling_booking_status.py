...
def build_classifiers():
    ...
    #         n_estimators=100, random_state=42, n_jobs=-1, verbosity=0, use_label_encoder=False, eval_metric="mlogloss",
    #     )
...
def evaluate_on_test(clf, X_train, y_train, X_test, y_test):
    ...
    auc = roc_auc_score(y_test, y_proba, multi_class="ovr", average="weighted") if y_proba is not None else None
...
def _():
    mo.md("""
    # Booking Status Prediction Model

    This notebook builds a **classification** model to predict **booking_status** (cancelled/completed/etc.) for ride-hailing trips.

    **Target:** `booking_status_enc` (0, 1, 2 - 3 classes)

    **Metrics:** Accuracy, F1 (weighted), AUC (OvR), Confusion Matrix
...
@app.cell
def _():
    df_raw = pd.read_csv(PROCESSED_DATA_DIR / "features_booking_status.csv")
...
def _(metrics):
    ...
    cm_df = pd.DataFrame(cm, index=["Pred 0", "Pred 1", "Pred 2"], columns=["True 0", "True 1", "True 2"])
    cm_df
    return
...
def _(final_pipe, tuned_pipe):
    ...
    save_dir = MODELS_DIR / "booking_status"
...
def _():
    ...
    mo.md("Configure the inputs below and click **Predict Status** to get a prediction.")
    return
...
def _():
    ...
    predict_btn = mo.ui.run_button(label="Predict Status")
...
def _(feature_cols, input_widgets, predict_btn, tuned_pipe):
    ...
    if predict_btn.value and tuned_pipe is not None:
        ...
        pred_md = (
            f"### Predicted Booking Status: **{pred_label}** (class {pred})\n\n"
            f"**Confidence:** {proba[pred]:.1%}\n\n"
            f"| Class | Probability |\n|-------|-------------|\n"
            f"| Cancelled (0) | {proba[0]:.1%} |\n"
            f"| Completed (1) | {proba[1]:.1%} |\n"
            f"| Incomplete (2) | {proba[2]:.1%} |\n\n"
            f"**Inputs:**\n"
        ...
        pred_md = "Configure inputs and click **Predict Status**."
    return (pred_md,)
